...
class User extends MY_Controller {
	...
	public function index() {
		...
		$users=$this->get_list();
		$this->_addition_display=['users'=>$this->get_list()];
		parent::index();
	}
	...
	protected function get_detail($id) {
		...
		$this->db->where('usr.id_pengadilan', $id);
		$t_user = $this->db->get();
	...
	function deletes()
  {
	$id_pengadilan = $this->uri->segment(4);
	
    $this->user_model->deletes($id_pengadilan);
	
	//return $this->flash_message('Hapus data berhasil.', 'SUCCESS');
	...
	public function cari(){
		...
		$data['users']=$this->user_model->cari($keyword);
