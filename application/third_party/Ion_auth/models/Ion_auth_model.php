...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			// TODO : Verify ReCaptcha atau diluar proses ini (ga bisa post klo blm bener) --
	...
	public function increase_login_attempts($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			return $this->login_attempts_model->insert($data);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		$this->response = $this->db->get($this->tables['users'] . ' AS ' . $this->tables['users']);
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
		                ...
		                ->join($this->tables['groups'] . ' AS ' . $this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id')
		                ->get($this->tables['users_groups'] . ' AS ' . $this->tables['users_groups']);
	}
