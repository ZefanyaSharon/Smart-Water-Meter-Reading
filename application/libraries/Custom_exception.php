...
class Custom_exception {
	 ...
	 */
	public function handle_exception($exception, $callback = null) {
		...
		if (ENVIRONMENT == 'production') {
			 ...
			 */
			if ($callback !== null) {
				call_user_func_array($callback, [$exception, $message]);
			} elseif (!empty($this->_default_callback)) {
				$callback_name = $this->_default_callback;
				$callback_name($exception, $message);
			} else {
				redirect(base_url());
			}
		} else {
			// set_status_header(500);
			$dump = $this->_pretty_dump($log, $log['message'], true);
			if ($callback !== null) {
				call_user_func_array($callback, [$exception, $dump]);
			} elseif (!empty($this->_default_callback)) {
				$callback_name = $this->_default_callback;
				$callback_name($exception, $dump);
			} else {
				redirect(base_url());
			}
			// exit;
	...
	private function _get_stacktrace() {
		...
		foreach ($trace as $entry_id => $entry) {
			...
			// if (isset($entry['args']) && ! empty(isset($entry['args'])))
			// 	$trace_full .= $this->_get_message_stacktrace($entry['args']);
		}
	...
	private function _get_message_stacktrace($args) {
		$result = '';
		foreach ($args as $arg) {
			$result .= $arg . PHP_EOL;
		}
