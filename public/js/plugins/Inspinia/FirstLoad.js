...
function jqgrid_window_fixed_width(id) {
	// return jQuery('#nah').width();
	// return jQuery('#gbox_' + id + '>parent').width(); // not working
	return jQuery('#' + id + '_container').width();
}
...
function jqgrid_boolean_invers_formatter(cellvalue, options, rowObject) {
	return boolean_invers_selectors[cellvalue];
}
...
function data_formater(cellvalue, options, rowObject) {
	var arr = options.colModel.formatoptions
	result = '<table class="border"><tbody>';
...
function jqgrid_column_created_updated(table_id, options) {
	options = options || {};
	var defaults = {
		width: 120,
		frozen: false,
	...
	return jqgrid_column_date(tableId, options);
}
...
function jquery_ready_load() {
	...
	$.jgrid.defaults.loadtext = '<div id="blocker" class="sk-spinner sk-spinner-three-bounce"><div class="sk-bounce1"></div><div class="sk-bounce2"></div><div class="sk-bounce3"></div></div>';
...
function jquery_blockui(jq_element, message) {
	if (message == undefined)
		message = '<div id="blocker" class="sk-spinner sk-spinner-three-bounce"><div class="sk-bounce1"></div><div class="sk-bounce2"></div><div class="sk-bounce3"></div></div>';
...
function jquery_ajax_error_handler(xhr, exception) {
	...
	// agar message tidak double maka dicari index dari } untuk item yang pertama
	var _index_string = xhr.responseText.indexOf('}');
	var message = xhr.responseText.substr(0, xhr.responseText.length - _index_string - 1);

	var _error_message = "";
	if (message.substr(0, 1) === '{' && message.substr(_index_string) === '}') {
		_error_message = jQuery.parseJSON(message).message;
	} else {
		_error_message = _status_error + (_server_error ? "<br/>" + _server_error : '');
	}
...
function action_formater(cellvalue, options, rowObject) {
	var btn_detail = '';
	var btn_edit = '';
	var btn_delete = '';
	var btn_metadata = '';
	var btn_put_metadata = '';
	var btn_reindex_es = '';
	var btn_reindex_druid = '';

	if (rowObject._source)
		btn_detail = '<a href="' + url + '/detail/' + rowObject._source.id + '"><button class="btn btn-info btn-xs" style="width:55px; margin:2px;" type="button"><i class="fa fa-list-ul"> Detil</i></button></a>';
	else
		btn_detail = '<a href="' + url + '/detail/' + rowObject['id'] + '"><button class="btn btn-info btn-xs" style="width:55px; margin:2px;" type="button"><i class="fa fa-list-ul"> Detil</i></button></a>';

	if (is_can_edit == true)
		btn_edit = '<a href="' + url + '/form?id=' + rowObject['id'] + '"><button class="btn btn-warning btn-xs" style="width:55px; margin:2px;" type="button"><i class="fa fa-pencil"> Ubah</i></button></a>';
	if (is_can_delete == true)
		btn_delete = '<button id="btnDelete_' + rowObject['id'] + '" data-id="' + rowObject['id'] + '" class="btnDelete btn btn-danger btn-xs" style="width:55px; margin:2px;" type="button"><i class="fa fa-trash"></i> Hapus</button>';
	if (typeof is_can_metadata !== 'undefined')
		btn_metadata = '<a href="' + url + '/assign_metadata?id=' + rowObject['id'] + '"><button class="btn btn-warning btn-xs" style="width:100px; margin:2px;" type="button"><i class="fa fa-pencil"> Assign Metadata</i></button></a>';
	if (typeof put_metadata !== 'undefined')
		btn_put_metadata = '<a href="' + url + '/form_metadata?id=' + rowObject['id'] + '"><button class="btnMetadata btn btn-primary btn-xs" style="width:70px; margin:2px;" type="button"><i class="fa fa-plus"> Metadata</i></button></a>';
	if (typeof reindex_es !== 'undefined')
		btn_reindex_es = '<button id="btnES_' + rowObject['id'] + '" data-id="' + rowObject['id'] + '" class="btnES btn btn-primary btn-xs" style="width:70px; margin:2px;" type="button"><i class="fa fa-plus"></i> Reindex</button>';
	if (typeof reindex_druid !== 'undefined')
		btn_reindex_druid = '<a href="' + url + '/reindex_druid?id=' + rowObject['id'] + '"><button class="btnDruid btn btn-info btn-xs" style="width:55px; margin:2px;" type="button"><i class="fa fa-plus"> Druid</i></button></a>';

	return btn_put_metadata + btn_detail + btn_edit + btn_delete + btn_metadata + btn_reindex_es + btn_reindex_druid;
}
